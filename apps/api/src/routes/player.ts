...
import { scrobbles, tracks, albums, track_artists, artists } from '@playbacc/types/db/schema'
...
player.get('/recently-played', async (ctx) => {
	...
	try {
		...
		const trackArtistsData = trackIds.length > 0 
			? await db
				.select({
				...
				.from(track_artists)
				.innerJoin(artists, eq(track_artists.artist_id, artists.id))
		...
		const items = recentScrobbles.map((scrobble) => ({
			played_at: scrobble.played_at?.toISOString() ?? new Date().toISOString(),
